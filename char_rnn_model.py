...
class CharRNN(object):
	...
	def __init__(self, is_training, batch_size, num_unrollings, vocab_size,
							 ...
							 model, dropout, input_dropout, decay_rate, max_learning_rate, min_learning_rate, use_batch=True):
		...
		if is_training:
			self.learning_rate = tf.train.exponential_decay(max_learning_rate, self.global_step, 1, decay_rate)
			...
			self.learning_rate = tf.constant(max_learning_rate)
	...
	def run_epoch(self, session, data_size, batch_generator, is_training, verbose=0, freq=10, summary_writer=None, debug=False, divide_by_n=1):
		...
		if epoch_size // divide_by_n == 0:
			error = 'Save frequency is too large for epoch size, maximum save frequency is: ' + str(epoch_size)
			return
		...
		return ppl, summary_str, global_step, lr
