...
def main():
    ...
    # Do we want to use the latest model rather than the best model
    parser.add_argument('--latest', dest='latest', action='store_true',
                        help='When used alongside init_dir, it automatically chooses to use the latest model')
    ...
    if args.model_path:
        sample_model = args.model_path
    else:
        if args.latest:
            sample_model = os.path.join(args.init_dir, 'saved_models/' + result['latest_model'])
        else:
            sample_model = os.path.join(args.init_dir, 'best_model/' + result['best_model'])
    ...
    if args.evaluate:
        ...
        with tf.Session(graph=graph) as session:
            saver.restore(session, sample_model)
